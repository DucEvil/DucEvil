window.onload = function () {
	...
	var tags = ["Áo Hoodie", "Áo Sweater", "Áo Khoác", "Quần jean"];
	...
	if (filters.length) { // có filter
		...
		addKhungSanPham('BEST SALE', yellow_red, ['star=3', 'sort=rateCount-decrease'], soLuong, div);
		addKhungSanPham('SẢN PHẨM MỚI', blue, ['promo=moiramat', 'sort=rateCount-decrease'], soLuong, div);
		addKhungSanPham('GIẢM GIÁ SỐC', yellow_red, ['promo=giamgia'], soLuong, div);

	}

	// Thêm chọn mức giá
	addPricesRange(0, 150);
	addPricesRange(150, 250);
	addPricesRange(250, 400);
	addPricesRange(400, 600);
	addPricesRange(600, 0);
...
function priceToString(min, max) {
	if (min == 0) return 'Dưới ' + max +"k" ;
	if (max == 0) return 'Trên ' + min +"k";
	return 'Từ ' + min  + ' - ' + max +"k";
}
